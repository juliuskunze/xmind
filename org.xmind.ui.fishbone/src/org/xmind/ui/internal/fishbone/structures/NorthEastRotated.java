...
public class NorthEastRotated extends AbstractSubFishboneDirection {

    public NorthEastRotated() {
        super(-Fishbone.RotateAngle, false, false, true, EAST, WEST);
    }
    ...
    public void fillFishboneData(IBranchPart branch, FishboneData data,
        ...
        PrecisionPoint joint = origin.getTranslated(0, hTopicIns.bottom);
        ...
        int insIndex = insertion == null ? -1 : subbranches.size()
                - insertion.getIndex();
...
//            PrecisionInsets preciseIns = h.ri(new PrecisionInsets(ins));
            ...
            if (bsa instanceof SubFishboneStructure) {
                ...
                FishboneData subData = sa.getCastedData(subBranch)
                        .getFishboneData();
            ...
            rhBranchBounds.union(hChildBranchIns.getBounds(rhChildRef).expand(
                    hChildBorder));
            double dx3 = Math.abs((hChildBorder.top + hChildBranchIns.top)
                    / sin);
