...
//            super.setNavCaches(sources, target, navType);
    ...
    protected IPart findNextOrPrev(IPart source, boolean nextOrPrev) {
        if (source instanceof ITopicPart) {
            return findNextOrPrevTopic((ITopicPart) source, nextOrPrev);
        }
        return super.findNextOrPrev(source, nextOrPrev);
    }

    private IPart findNextOrPrevTopic(ITopicPart current, boolean nextOrPrev) {
        if (nextOrPrev)
            return findSucceddingTopicPart(current);
        return findPrecedingTopicPart(current);
    }

    private IPart findSucceddingTopicPart(ITopicPart current) {
        IBranchPart branch = current.getOwnerBranch();
        if (branch != null) {
            return findSucceedingBranch(branch).getTopicPart();
        }
        return current;
    }

    private IBranchPart findSucceedingBranch(IBranchPart current) {
        IBranchPart child = findFirstChild(current);
        if (child != null)
            return child;
        return findSucceedingSiblingOrAncestor(current);
    }
            ...
            if (current == sheet.getCentralBranch()) {
                ...
                if (index >= 0) {
                    if (index < floatingBranches.size() - 1) {
                        return floatingBranches.get(index + 1);
                    }
    ...
    private IPart findPrecedingTopicPart(ITopicPart current) {
        IBranchPart branch = current.getOwnerBranch();
        if (branch != null) {
            IBranchPart prev = findPrecedingBranch(branch);
            return prev.getTopicPart();
        }
        return current;
    }
    ...
    private IBranchPart findPrecedingBranch(IBranchPart current) {
        IBranchPart parent = current.getParentBranch();
        if (parent != null) {
            List<IBranchPart> branches = parent.getSummaryBranches();
            int index = branches.indexOf(current);
            if (index > 0) {
