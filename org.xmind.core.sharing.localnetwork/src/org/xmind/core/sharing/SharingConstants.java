/* ******************************************************************************
 * Copyright (c) 2006-2012 XMind Ltd. and others.
 * 
 * This file is a part of XMind 3. XMind releases 3 and
 * above are dual-licensed under the Eclipse Public License (EPL),
 * which is available at http://www.eclipse.org/legal/epl-v10.html
 * and the GNU Lesser General Public License (LGPL), 
 * which is available at http://www.gnu.org/licenses/lgpl.html
 * See http://www.xmind.net/license.html for details.
 * 
 * Contributors:
 *     XMind Ltd. - initial API and implementation
 *******************************************************************************/
package org.xmind.core.sharing;

/**
 * 
 * @author Frank Shaka
 * 
 */
public interface SharingConstants {

    String PLUGIN_ID = "org.xmind.core.sharing"; //$NON-NLS-1$

    String COMMAND_SOURCE = "sharing.core.xmind.org"; //$NON-NLS-1$

    String COMMAND_VERSION = "1"; //$NON-NLS-1$

    String PROP_VERSION = "version"; //$NON-NLS-1$

    String PROP_REMOTE = "remote"; //$NON-NLS-1$

    String PROP_NAME = "name"; //$NON-NLS-1$

    String PROP_MAPS = "maps"; //$NON-NLS-1$

    String PROP_ID = "id"; //$NON-NLS-1$

    String PROP_THUMBNAIL = "thumbnail"; //$NON-NLS-1$

    String PROP_MISSING = "missing"; //$NON-NLS-1$

    String PROP_MAP = "map"; //$NON-NLS-1$

    String PROP_TYPE = "type"; //$NON-NLS-1$

    String PROP_CONTENT = "content"; //$NON-NLS-1$

    int CODE_NOT_FOUND = 404;

    int CODE_MISSING_ARGUMENT = 400;

    String PREF_LOCAL_LIBRARY_NAME = "localLibraryName"; //$NON-NLS-1$

}
