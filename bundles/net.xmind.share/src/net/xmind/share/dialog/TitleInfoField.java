...
public class TitleInfoField {
    ...
    public TitleInfoField(boolean grabVerticalSpace, boolean readOnly,
            boolean singleLine) {
    ...
    private void createTitleLabel(Composite parent) {
        canvas = new FigureCanvas(parent, SWT.BORDER);
        ...
        canvas.setLayoutData(textData);

        text = new RotatableWrapLabel(getTextStyle());
        text.setSingleLine(true);
        text.setAbbreviated(true);
        text.setEnabled(true);
        text.setRequestFocusEnabled(true);
        canvas.setContents(text);
//        text.setText(getTitleText());
    ...
    public boolean isDisposed() {
        return text != null || label.isDisposed();
    }
