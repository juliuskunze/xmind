...
public class GrayedGraphics extends Graphics {
    ...
    protected Color getWorkingColor(Color c) {
        ...
        if (grayed == null) {
            grayed = createGrayed(c.getDevice(), rgb);
            workingColors.put(rgb, grayed);
        }
        return grayed;
    }

    private Color createGrayed(Device device, RGB rgb) {
        int l = lightness(rgb.red, rgb.green, rgb.blue);
        l += (0xff - l) / 2;
        return new Color(device, new RGB(l, l, l));
    }
