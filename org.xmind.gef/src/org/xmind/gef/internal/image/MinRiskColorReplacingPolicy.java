...
//    private Map<RGB, Map<RGB, Integer>> riskCache;
    ...
    /**
     * @see cn.brainy.gef.image.IColorReplacingPolicy#getReplacingColors(int, java.util.Map)
     */
    public RGB[] getReplacingColors( int numMaxColors, final Map<RGB, Integer> colorOccurrences ) {
        ...
        for ( int i = 0; i < sortedColors.size(); i++ ) {
            ...
            ColorReplacing rep = createMinRiskReplacing( c1, sortedColors.subList( 0, i ) );
        ...
        Set<RGB> sorter2 = new TreeSet<RGB>( new Comparator<RGB>() {
             ...
             */
            public int compare( RGB o1, RGB o2 ) {
                int delta = replacings.get( o2 ).risk * (int) Math.sqrt( colorOccurrences.get( o2 ) ) - 
                    replacings.get( o1 ).risk * (int) Math.sqrt( colorOccurrences.get( o1 ) );
